...
use std::hash::{Hash, Hasher};
 // TODO: delete this line for Milestone 4
use std::{fmt, fs};
...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let name = OpenFile::path_to_name(fs::read_link(format!("/proc/{}/fd/{}", pid, fd)).ok()?.to_str()?);
        let str = fs::read_to_string(format!("/proc/{}/fdinfo/{}", pid, fd)).ok()?;
        let cursor = OpenFile::parse_cursor(str.as_str())?;
        let access_mode = OpenFile::parse_access_mode(str.as_str())?;
        // let result = OpenFile{}
